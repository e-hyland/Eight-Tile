...
void fileread(FILE *fp, char board[SIZE][SIZE])
{
   ...
   fclose(fp);
   validboard(board);   
   solvable(board); /*Check if board is not solvable*/
...
/*Flattens board into a string so that we can count the number of inversions*/
void solvable(char board[SIZE][SIZE])
{
   ...
   for (i=0;i<SIZE;i++){
      for (j=0;j<SIZE;j++){
         if (board[i][j]!=' '){ /*Do not take into account space for inversion count*/
            string[k] = board[i][j];
            k++;
         }
      }
...
/*Inversions = number of times the cell previous is larger than the next cell. If this number is odd, the board is unsolvable*/
void inversions(char string[CELLS])
{
   int j, i, cnt=0;

   for(j=0;j<CELLS-1;j++){
      for(i=j+1;i<CELLS-1;i++){
...
void iterate(int *x, int *y, Node *front)
{
   ...
   if (current->next==NULL){
      fprtinf(stderr, "Please try again.\n");
   }
